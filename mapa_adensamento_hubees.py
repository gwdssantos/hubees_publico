...
# === LISTA DE EPs ATIVOS POR BAIRRO ===
def _is_active(x):
    return str(x).strip().lower() in {"ativo", "active", "ativado", "sim", "true", "1", "yes"}

# anexa o nome do bairro a cada EP dentro das zonas prioritárias
_bairros_min = gdf_all_bairros[['Name', 'geometry']].rename(columns={'Name': 'BAIRRO'})
eps_com_bairro = gpd.sjoin(
    eps_dentro, 
    _bairros_min, 
    how="inner", 
    predicate="within"
...
eps_ativos_por_bairro = eps_com_bairro[eps_com_bairro['ACTIVE'].apply(_is_active) if 'ACTIVE' in eps_com_bairro.columns else False]

# seleciona e ordena colunas para saída
cols_saida = []
for c in ['BAIRRO','NOME','EP_NAME','ID','ACTIVE','EP_NETPARK','LAT','LNG']:
    if c in eps_ativos_por_bairro.columns:
        cols_saida.append(c)
# garante que BAIRRO vem primeiro
cols_saida = (['BAIRRO'] + [c for c in cols_saida if c != 'BAIRRO']) if 'BAIRRO' in cols_saida else cols_saida

lista_eps_ativos = eps_ativos_por_bairro.copy()
if cols_saida:
    lista_eps_ativos = lista_eps_ativos[cols_saida]
lista_eps_ativos = lista_eps_ativos.sort_values(['BAIRRO'] + ([ 'NOME' ] if 'NOME' in lista_eps_ativos.columns else []))

# salva CSV com a lista
lista_eps_ativos.to_csv("eps_ativos_por_bairro.csv", index=False, encoding="utf-8-sig")

# (opcional) imprime um resumo rápido no console
print("Resumo EPs Ativos por Bairro:")
print(lista_eps_ativos.groupby('BAIRRO').size().reset_index(name='QTD'))

# === ENUMERAR QUADRANTES POR BAIRRO (AJUSTE FEITO AQUI)
estatisticas_bairros = []

for _, bairro_row in gdf_all_bairros.iterrows():
    bairro_nome = bairro_row['Name']

    # GERA O GRID SÓ PARA ESSE BAIRRO:
    pol_bairro = gpd.GeoDataFrame(geometry=[bairro_row.geometry], crs="EPSG:4326").to_crs(epsg=3857)
    minx, miny, maxx, maxy = pol_bairro.geometry.iloc[0].bounds

    grid_cells = []
    for x in np.arange(minx, maxx, STEP):
        for y in np.arange(miny, maxy, STEP):
            cell = box(x, y, x + STEP, y + STEP)
            intersecao = cell.intersection(pol_bairro.geometry.iloc[0])
            if not intersecao.is_empty:
                grid_cells.append(intersecao)

    gdf_quadrantes_bairro = gpd.GeoDataFrame(geometry=grid_cells, crs="EPSG:3857").to_crs("EPSG:4326")
    quads_do_bairro = gdf_quadrantes_bairro
...
for linha in estatisticas_bairros:
    ...
    nome = linha.split(":")[0].strip().replace("_", " ").upper()
    ...
    <div style="background:#f9f9f9; border-radius:8px; padding:10px 12px; margin-bottom:10px; border-left:3px solid #666;">
        <div style="font-size:12px; font-weight:normal; margin-bottom:6px;">
...
##Gravar os dados no Azure
import pyodbc
from datetime import datetime
import re

# --- BANCO ---
server = 'netparksqlcloud.database.windows.net'
database = 'NPK_BI_PROD'
username = 'pbi_admin'
password = '8vbN5$yYjk10'
...
# --- DATA ---
DATA = datetime.now().date()  # ou data de referência do dashboard

# --- DELETE registros existentes para a data ---
cursor.execute("DELETE FROM STG_HUB_EPS_ADENSAMENTO WHERE DATA = ?", (DATA,))
print(f"Registros da data {DATA} excluídos com sucesso!")

# --- LOOP PELAS LINHAS DA LEGENDA ---
for linha in estatisticas_bairros:
    if "Quadrantes Válidos" not in linha:
        continue  # Pula linhas que não trazem dados

    # Extrai o nome do bairro
    match_nome = re.search(r"<b>([^:]+):</b>", linha)
    if match_nome:
        ZP = match_nome.group(1).strip()
    else:
        continue

    # Extrai os valores por expressão regular
    match_quad_validos = re.search(r"Quadrantes Válidos \[(\d+)\]", linha)
    match_quad_com_ep = re.search(r"Com EP \[(\d+)\]", linha)
    match_total_eps = re.search(r"Total EPs \[(\d+)\]", linha)
    match_zr = re.search(r"Com \+1 EP ou Netpark \[(\d+)\]", linha)

    if match_quad_validos and match_total_eps and match_quad_com_ep:
        TOTAL_QUADRANTES = int(match_quad_validos.group(1))
        EPS_MAPEADOS = int(match_total_eps.group(1))
        QUADRANTES_COM_COBERTURA = int(match_quad_com_ep.group(1))
        QUADRANTES_ZR = int(match_zr.group(1)) if match_zr else 0

        # Insert para cada bairro
        query = """
            INSERT INTO STG_HUB_EPS_ADENSAMENTO (ZP, DATA, TOTAL_QUADRANTES, EPS_MAPEADOS, QUADRANTES_COM_COBERTURA, QUADRANTES_ZR)
            VALUES (?, ?, ?, ?, ?, ?)
        """
        cursor.execute(query, (ZP, DATA, TOTAL_QUADRANTES, EPS_MAPEADOS, QUADRANTES_COM_COBERTURA, QUADRANTES_ZR))
        print(f"{ZP}: {TOTAL_QUADRANTES}, {EPS_MAPEADOS}, {QUADRANTES_COM_COBERTURA},{QUADRANTES_ZR}")

conn.commit()
cursor.close()
conn.close()
print("Todos os bairros gravados com sucesso!")
